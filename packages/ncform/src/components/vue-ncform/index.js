...
module.exports = {
  ...
  install: (Vue, options = { extComponents: {}, extRules: [], lang: "" }) => {
    ...
    _map(
      ...
      (compItem, name) => {
        window.__$ncform.__ncformComponents[_kebabCase(name)] = compItem;
        Vue.component(`ncform-${_kebabCase(name)}`, compItem);
      }
    );
    ...
    Vue.prototype.$ncformValidate = formName => {
      ...
      return vm.ncformValidate().then(data => {
        ...
        if (!data.result && config.enabled) {
          vm.$nextTick(() => {
            const firstErrorElem = Array.prototype.slice
              .call(vm.$el.querySelectorAll(".invalid-feedback"))
              .find(elem => elem.style.display !== "none");
            if (firstErrorElem) {
    ...
    Vue.prototype.$ncformAddWidget = ({ name, widget }) => {
      window.__$ncform.__ncformComponents[_kebabCase(name)] = widget;
      Vue.component(`ncform-${_kebabCase(name)}`, widget);
    };

    Vue.prototype.$ncformAddRule = ({ name, rule }) => {
      const ruleItem = {};
      ruleItem[name] = rule;
      window.__$ncform.__ncformRegularValidation.registerRule(ruleItem);
    };

    Vue.prototype.$ncformAllRules = () =>
      Object.keys(window.__$ncform.__ncformRegularValidation.allRules);
