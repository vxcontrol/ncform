  ...
  <div class="__array-table-form-item">
    ...
    <table v-show="!collapsed" class="table table-bordered">
      ...
      <colgroup v-else>
        ...
        <col v-if="globalStatus !== 'preview' && showActionColumn && !disabled" width="130px"/>
      </colgroup>
      <thead>
          ...
          <th v-if="globalStatus !== 'preview' && showActionColumn && !disabled">{{$nclang('action')}}</th>
      </thead>
      <tbody>
        <tr v-for="(dataItem, idx) in schema.value" :key="dataItem.__dataSchema.__id">
          ...
          <td v-if="globalStatus !== 'preview' && showActionColumn && !disabled">
            <!-- 项控制按钮 -->
      ...
      <tfoot v-if="globalStatus !== 'preview' && (!mergeConfig.disableDel || !mergeConfig.disableAdd) && !disabled">
        <tr>
