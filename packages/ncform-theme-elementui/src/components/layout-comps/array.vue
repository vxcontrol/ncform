  ...
  <div class="__array-form-item">
    ...
    <div v-show="!collapsed" v-for="(dataItem, idx) in schema.value" :key="dataItem.__dataSchema.__id" class="list-item">
      ...
      <div class="list-item-label">
        <label>{{_analyzeVal(dataItem.__dataSchema.ui.label, (idxChain ? idxChain + ',' : '') + idx) + (schema.ui.showIdxLabel ? ' ' + (idx + 1) : '')}}</label>

        <!-- 项控制按钮 -->
        <div class="el-button-group">
          <button @click="collapseItem(dataItem.__dataSchema)" v-show="dataItem.__dataSchema._expand" v-if="!mergeConfig.disableItemCollapse && !disabled" :disabled="disabled" type="button" class="el-button el-button--mini"><i class="el-icon-arrow-down"></i></button>
          <button @click="collapseItem(dataItem.__dataSchema)" v-show="!dataItem.__dataSchema._expand" v-if="!mergeConfig.disableItemCollapse && !disabled" :disabled="disabled" type="button" class="el-button el-button--mini"><i class="el-icon-arrow-up"></i></button>
          <template v-if="globalStatus !== 'preview' && !disabled">
    ...
    <!-- 列表控制按钮 -->
    <div v-show="!collapsed" class="el-button-group" v-if="globalStatus !== 'preview' && (!mergeConfig.disableAdd || !mergeConfig.disableDel) && !disabled">
